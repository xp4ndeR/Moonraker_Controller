...
async def async_setup_entry(hass : HomeAssistant, config_entry : ConfigEntry, async_add_entities : AddEntitiesCallback) -> None :
    ...
    for x in CLIMATES_LIST:
        entities.append(MoonrakerClimateBase(
            coordinator,
            device_id,
            x["climate_type"],
            x["climate_target"],
...
class MoonrakerClimateBase(CoordinatorEntity, ClimateEntity):
    ...
    def hvac_mode(self)-> HVACMode | str | None:
        """Set HVAC mode."""
        return HVACMode.HEAT if getattr(self.coordinator.printer,self._climate_target) != 0 else HVACMode.OFF
    ...
    async def async_set_temperature(self, **kwargs):
        ...
        try:
            await self.coordinator.moonraker.push_data(self._gcode.format(kwargs[ATTR_TEMPERATURE]))
